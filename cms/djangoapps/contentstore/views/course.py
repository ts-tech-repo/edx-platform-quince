    ...
    try:
        ...
        if source_course_key:
            ...
            try:
                ...
                api_data = {"wstoken" : configuration_helpers.get_value("MOODLE_TOKEN", ""), "wsfunction" : "core_course_create_courses", "moodlewsrestformat" : "json", "courses[0][fullname]" : display_name, "courses[0][categoryid]" : 1, "courses[0][shortname]" : "{0}|{1}".format(course, run), "courses[0][summary]" : "", "courses[0][customfields][0][shortname]" : "sites", "courses[0][customfields][0][value]" : org, "courses[0][customfields][1][shortname]" : "instances", "courses[0][customfields][1][value]" : settings.CMS_BASE, "courses[0][startdate]" : new_course.start.astimezone(tz.gettz("Asia/Kolkata")).strftime("%s"), "courses[0][enddate]" : new_course.end.strftime("%s") if new_course.end is not None else 0}
...
def _refresh_course_tabs(user: User, course_block: CourseBlock):
    ...
    #SA | progressTabIssueFix
    # Update ProgressTab's is_hidden value to True | by default it should be False
