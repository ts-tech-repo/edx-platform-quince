...
from lms.djangoapps.courseware.courses import get_course_assignments, get_courses, get_first_component_of_block, sort_by_announcement, sort_by_start_date
...
def extras_get_payment_details(request):
    ...
    student_orders_endpoint = "https://staging.drupal.talentsprint.com/get-payment-history.dpl"
...
def extras_certificate(request):
    try:
        ...
        r = requests.post("https://cdn.exec.talentsprint.com/app/certificates", headers = {'content-type': 'application/json'}, data = json.dumps(data))
...
def extras_get_assessment_details(request):
    ...
    context = {"id" : course_id, "display_name" : course_details.display_name, "timecreated" : course_details.start, "timemodified" : course_details.subtree_edited_on.replace(tzinfo=None).replace("T", " "),  "activities" : []}
    for assignment in get_course_assignments(course_key, user):
        context["activities"].append({
            ...
            "timemodified" : assignment.block_data.get_xblock_field(assignment.block_key, 'subtree_edited_on').replace(tzinfo=None).replace("T", " ")
